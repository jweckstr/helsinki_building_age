...
plat = urlParams.get('lat') || 60.17194
plon = urlParams.get('lon') || 24.94138
pzoom = urlParams.get('zoom') || 17
pyear = urlParams.get('year')  || 2030

slider_now = Math.min(Math.max(pyear, slider_min), slider_max)
...
refresh_params = ->
    update_params("lat", map.getCenter()["lat"].toFixed(4))
    update_params("lon", map.getCenter()["lng"].toFixed(4))
    update_params("zoom", map.getZoom().toFixed(1))
...
osm_layer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
		maxZoom: 18,
		id: 'mapbox.streets')
sp_layer = L.tileLayer.wms(GEOSERVER_BASE_URL + '/ows?',
...
slider.on 'slide', (ev) ->
    ...
    update_params("year", ev.value)
    update_screen ev.value

slider.on 'slideStop', (ev) ->
    if not animating
        refresh_buildings()
...
transport_styler = (feat) ->
    ...
    category = parseInt feat.properties.kategoria || -1
...
get_finna2 = (address, max_imgs, hash_code="#finna_images", aika_alku='0', aika_loppu='9999') ->
    $.ajax
        ...
        success: (data, textStatus, jqXHR) ->
          ...
          #img_urls = (if x.images.length then "https://finna.fi" + x.images[0] else "" for x in data.records )
          img_urls = []
          for x in data.records
            if x.images.length
              img_urls.push "https://finna.fi" + x.images[0]
          img_html = ""
...
refresh_buildings = ->
    ...
    get_wfs 'hfors:all_merged',
        maxFeatures: 50000
        bbox: str
        propertyName: 'rakvuosi_a,rakvuosi_l,purvuosi_a,purvuosi_l,suunnittel,Nimi,Osoite,the_geom'
        , (data) ->
            if building_layer
                map.removeLayer building_layer
    ...
    get_wfs 'hfors:trafik',
        maxFeatures: 10000
        bbox: str
        propertyName: 'rak_alku,rak_loppu,pur_alku,pur_loppu,kategoria,the_geom'
        , (data) ->
            if transport_layer
                map.removeLayer transport_layer
            transport_layer = L.geoJson data,
                ...
                onEachFeature: (feat, layer) ->
                    ...
                    category = feat.properties.kategoria
...
map.on 'moveend', refresh_params

$(".infobut").click ->
    $(".infodiv").slideToggle()


overlayMaps =  {"Smith-Polvisen liikennesuunnitelma 1968": sp_layer, "Open Street Map": osm_layer}
...
$("#play-btn").click ->
    if animating
        ...
        int = setInterval () ->
                ...
                if current_state.year_built_first < 1900 and current_state.year_built_first >= 1840
                    sleep 250
                if current_state.year_built_first < 1840
                    sleep 0
                if current_state.year_built_first >= 1900
                    sleep 500
                ...
                if newyear <= slider_max
                    ...
                    $("#play-btn").html "&#9658;"
                    animating = false
            , 0  # this is the delay between years
